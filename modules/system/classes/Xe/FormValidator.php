...
class FormValidator {
	 ...
	 *        	验证规则,可以是多个,并且可以传参,例如 require|length(9,12)
	 * @param string $error_message
	 *        	自定义的出错信息,可选,如果给定将覆盖验证函数的默认出错信息
	 */
...
function form_validator_length($str,$data,$min,$max)
{
	$len=strlen(trim($str));
	return ($len>=$min) && ($len <=$max);
}
FormValidator::set_error ( 'equal', '重复输入不一致' );
// FormValidator::set_error ( 'equal', '%s的输入与%s不一致' );
FormValidator::set_error ( 'require', '%s不能为空' );
FormValidator::set_error ( 'length', '%s的长度须为%s至%s个字符' );
